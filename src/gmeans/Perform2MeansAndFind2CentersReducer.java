 ...
 * @author tibo
 */
public class Perform2MeansAndFind2CentersReducer implements Reducer<LongWritable, Point, NullWritable, NullWritable> {
    private JobConf job;
    ...
    public void reduce(
        ...
        if (center_id.get() < Perform2MeansAndFind2CentersMapper.OFFSET) {
            ...
            int new_center_id = (int) (center_id.get() - Perform2MeansAndFind2CentersMapper.OFFSET);
            writeCenterToCache("IT-" + (gmeans_iteration + 1) + "_CENTER-" + new_center_id, points.next());
    ...
    private void writeCenterToCache(String key, Point new_center) throws IOException {
        System.out.println("Writing center to " + key);
        MemcachedClient memcached = new MemcachedClient(
                new InetSocketAddress("127.0.0.1", 11211));
